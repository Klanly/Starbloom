...
namespace PixelCrushers.DialogueSystem.LoveHate
{
	...
	/// the Lua functions.
	/// </summary>
	[AddComponentMenu("Dialogue System/Third Party/Love\u2215Hate/LoveHateLua (on Dialogue Manager)")]
	public class LoveHateLua : MonoBehaviour 
	{
		...
		{
			return string.IsNullOrEmpty(actorName) ? GameObject.FindGameObjectWithTag("Player") : Tools.GameObjectHardFind(actorName);
		}
		                             ...
		                             double arousalChange, double dominanceChange)
		{
			...
			factionMember.pad.Modify((float) happinessChange, (float) pleasureChange, (float) arousalChange, (float) dominanceChange);
		}
