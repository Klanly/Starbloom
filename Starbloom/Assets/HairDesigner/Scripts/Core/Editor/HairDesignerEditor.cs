...
namespace Kalagaan
{
    namespace HairDesignerExtension
    {
        ...
        public class HairDesignerEditor : Editor
        {
            ...
            public void CreateColliderAndTpose()
            {
            ...
            public override void OnInspectorGUI()
            {
                ...
                {
                    bgStyleAlpha = new GUIStyle();
                    bgStyleAlpha.normal.background = new Texture2D(1, 1);
                    bgStyleAlpha.normal.background.SetPixel(0, 0, new Color(.1f,.1f,.1f,.3f));
                ...
                for (int i = 0; i < hd.m_generators.Count; ++i)
                {
                    ...
                    hd.m_generators[i].m_enable = EditorGUILayout.Toggle(hd.m_generators[i].m_enable, GUILayout.MaxWidth(20));
                    ...
                    if (!hd.m_generators[i].m_meshLocked 
                        && hd.m_generators[i].m_layerType != HairDesignerBase.eLayerType.FUR_SHELL )
                        allLocked = false;
                ...
                if (!Application.isPlaying)
                {
                    ...
                    if (m_addnewLayer)
                    {
                        ...
                        GUILayout.Label("Choose layer type", EditorStyles.centeredGreyMiniLabel);
                        if (GUILayout.Button(new GUIContent("Short hair/fur\n(Polygons)", (Icon(eIcons.BULLET)))))
                    ...
                    if (lt != HairDesignerBase.eLayerType.NONE)
                    {
                        ...
                        if (g != null)
                        {
                            ...
                            if (hd.m_generators[hd.m_generatorId].m_hairMeshMaterial == null)
                            {
                                hd.m_generators[hd.m_generatorId].m_hairMeshMaterial = lt != HairDesignerBase.eLayerType.FUR_SHELL ? hd.m_defaultMaterial : hd.m_defaultFurMaterial;
                            }
                    ...
                    {
                        EditorGUILayout.HelpBox("When polygons are done, lock all the layers", MessageType.Info);
                    }
                }
                GUILayout.Space(20);
                ...
                if (hd.generator != null)
                {
                    ...
                    Editor tmpEditor = Editor.CreateEditor(hd.generator, null);
                ...
                for (int i = 0; i < hd.m_generators.Count; ++i)
                {
                    ...
                    {
#if UNITY_5_5_OR_NEWER
                        EditorUtility.SetSelectedRenderState(hd.m_generators[i].m_meshInstance.GetComponent<MeshRenderer>(), EditorSelectedRenderState.Hidden );
                        EditorUtility.SetSelectedRenderState(hd.m_generators[i].m_meshInstance.GetComponent<SkinnedMeshRenderer>(), EditorSelectedRenderState.Hidden );
#else
                        EditorUtility.SetSelectedWireframeHidden(hd.m_generators[i].m_meshInstance.GetComponent<MeshRenderer>(), true);
                        EditorUtility.SetSelectedWireframeHidden(hd.m_generators[i].m_meshInstance.GetComponent<SkinnedMeshRenderer>(), true);
#endif
                    }
            ...
            void DrawSceneMenu( HairDesignerBase hd )
            {
                ...
                m_sceneMenuX = Mathf.Lerp(m_sceneMenuX, 0f, .05f);
                if (m_sceneMenuX < -1f)
