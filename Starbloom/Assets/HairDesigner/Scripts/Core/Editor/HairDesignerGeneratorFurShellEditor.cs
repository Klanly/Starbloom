                    ...
                    for (int rc = 0; rc < maxRay; ++rc)
                    {
                        ...
                        */
                        ...
                        foreach (RaycastHit hitInfo in hitInfos)
                        {
                            ...
                            Vector4 t0 = tangents[triangles[hitInfo.triangleIndex * 3 + 0]];
                            Vector4 t1 = tangents[triangles[hitInfo.triangleIndex * 3 + 1]];
                            Vector4 t2 = tangents[triangles[hitInfo.triangleIndex * 3 + 2]];
                            ...
                            if (m_currentTool == ePaintingTool.BRUSH)
                            {
                                ...
                                for (int i = 0; i < 3; ++i)
                                {
                                    vertexId = triangles[hitInfo.triangleIndex * 3 + i];
                                    if (vertexId != -1)
                                    {
                                        uv = uvs[vertexId];
                                        normal = normals[vertexId];
                                        tangent = tangents[vertexId];

                                        if (!brushVerticesApplied.Contains(vertexId))
                                        {
                                            if (!m_verticesLink.ContainsKey(vertices[vertexId]))
                                            {
                                                m_verticesLink = null;
                                                InitLinkedVertices();
                                            }
                                            ...
                                            for (int l = 0; l < m_verticesLink[vertices[vertexId]].Count; ++l)
                                            {
                                                //add merged vertices
                                                if (!brushVerticesApplied.Contains(m_verticesLink[vertices[vertexId]][l]))
                                                    brushVerticesApplied.Add(m_verticesLink[vertices[vertexId]][l]);
                                            }
                                    ...
                                    if (!m_CtrlMode && !m_ShiftMode)
                                    {
                                        ...
                                        if (vertexId != -1)
                                        {
                                            ...
                                            */
                                            ...
                                            for (int vid = 0; vid < ids.Count; ++vid)
                                            {
                                                normal = normals[ids[vid]];
                                                tangent = tangents[ids[vid]];
                                                //uv = uvs[ids[vid]];
                                                //uv.x = ((float)ids[vid] % (float)brushTex.width) + (1f/ (float)brushTex.width)*.5f;
                                                //uv.y = Mathf.Floor((float)ids[vid] / (float)brushTex.width);
                                                ...
                                                //brushTex.SetPixel((int)uv.x, brushTex.height-(int)uv.y, c);
                                                /*
                                                c.r = vProj.x;
                                                c.g = vProj.y;
                                                c.b = vProj.z;
                                                */
                                                //c.r = Mathf.Lerp(c.r, v.x * .5f + .5f, m_brushIntensity * falloff * .1f);
                                                //c.g = Mathf.Lerp(c.g, v.y * .5f + .5f, m_brushIntensity * falloff * .1f);
                                                //c.b = Mathf.Lerp(c.b, v.z * .5f + .5f, m_brushIntensity * falloff * .1f);
                                            }
                                        }
                                    }
                                    else if (m_CtrlMode && !m_ShiftMode)
