...
//
// DON'T touch defines below... (unless you know exactly what you're doing)
#if !defined(_4LAYERS) || defined(RTP_USE_COLOR_ATLAS)
	#define UV_BLEND_SRC_0 (UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01M.xy, _MixMipActual.xx)).rgba)
	#define UV_BLEND_SRC_1 (UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01M.zw, _MixMipActual.yy)).rgba)
	#define UV_BLEND_SRC_2 (UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23M.xy, _MixMipActual.zz)).rgba)
	#define UV_BLEND_SRC_3 (UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23M.zw, _MixMipActual.ww)).rgba)
	#define UV_BLEND_SRC_4 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01M.xy, _MixMipActual.xx)).rgba)
	#define UV_BLEND_SRC_5 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01M.zw, _MixMipActual.yy)).rgba)
	#define UV_BLEND_SRC_6 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23M.xy, _MixMipActual.zz)).rgba)
	#define UV_BLEND_SRC_7 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23M.zw, _MixMipActual.ww)).rgba)
#else
	#define UV_BLEND_SRC_0 (UNITY_SAMPLE_TEX2D_LOD(_SplatC0, float4(uvSplat01M.xy, _MixMipActual.xx)).rgba)
	#define UV_BLEND_SRC_1 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC1, _SplatC0, float4(uvSplat01M.zw, _MixMipActual.yy)).rgba)
	#define UV_BLEND_SRC_2 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC2, _SplatC0, float4(uvSplat23M.xy, _MixMipActual.zz)).rgba)
	#define UV_BLEND_SRC_3 (UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC3, _SplatC0, float4(uvSplat23M.zw, _MixMipActual.ww)).rgba)
#endif
...
UNITY_DECLARE_TEX2D(_SplatAtlasC);
UNITY_DECLARE_TEX2D_NOSAMPLER(_SplatAtlasB);

UNITY_DECLARE_TEX2D(_SplatC0);
UNITY_DECLARE_TEX2D_NOSAMPLER(_SplatC1);
UNITY_DECLARE_TEX2D_NOSAMPLER(_SplatC2);
UNITY_DECLARE_TEX2D_NOSAMPLER(_SplatC3);
...
UNITY_DECLARE_TEX2D(_BumpMap89);
UNITY_DECLARE_TEX2D_NOSAMPLER(_BumpMapAB);
...
UNITY_DECLARE_TEX2D(_TERRAIN_HeightMap3);
UNITY_DECLARE_TEX2D_NOSAMPLER(_TERRAIN_HeightMap2);
UNITY_DECLARE_TEX2D_NOSAMPLER(_TERRAIN_HeightMap);
...
float4 TERRAIN_WaterMetallic89AB;
...
// PBL / IBL
half4 RTP_metallic89AB, RTP_metallic4567;
half4 RTP_glossMin89AB, RTP_glossMin4567;
half4 RTP_glossMax89AB, RTP_glossMax4567;
	...
	void vert (inout appdata v) {
		...
		//
				...
				// we still benefit phong geometry smoothing
					...
					float4 splat_controlA = UNITY_SAMPLE_TEX2D_LOD(_Control3, float4(gUV, 0,0));
					float total_coverage=dot(splat_controlA, 1);
					#ifndef _4LAYERS
						float4 splat_controlB = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_Control2, _Control3, float4(gUV, 0,0));
				...
				//	#else
						#ifdef USE_EXTRUDE_REDUCTION
							tHA=saturate(lerp(UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, float4(_INPUT_uv.xy,0,detailMIPlevel)), 1, PER_LAYER_HEIGHT_MODIFIER89AB)+0.001);
						#else
							tHA=saturate(UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, float4(_INPUT_uv.xy,0,detailMIPlevel))+0.001);
						#endif	
				//	#endif
					splat_control1 *= tHA;
						...
						#ifdef USE_EXTRUDE_REDUCTION
							tHB=saturate(lerp(UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, float4(_INPUT_uv.xy,0,detailMIPlevel)), 1, PER_LAYER_HEIGHT_MODIFIER4567)+0.001);
							splat_control2 *= tHB;
						#else
							tHB=saturate(UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, float4(_INPUT_uv.xy,0,detailMIPlevel))+0.001);
							splat_control2 *= tHB;
						#endif	
...
//	return;
//	o.Emission.xy=frac(IN.uv_Control.xy*10+0.00001);//UNITY_SAMPLE_TEX2D_LOD(_ColorMapGlobal, float4(IN.uv_Control.xy,0,0)).rgb;
//	return;
	...
	float3 binormalBase = -cross( vertexNorm, tangentBase );
		...
		#define _ddyGlobal float2(0,0)
			...
			float2 _ddyGlobal=ddy(IN_uv_Control);
				...
				float4 tHeightmapTMP = UNITY_SAMPLE_TEX2D(_TERRAIN_HeightMap3, tiledUV); // potrzebujemy tego tymczasowo na koncu
			#endif						
		#else
			#ifdef UNITY_PASS_META	
...
//	return;
	...
	float2 dy = ddy( IN_uv);
		...
		//
			...
			#endif
		#else
			float4 splat_controlA = UNITY_SAMPLE_TEX2D(_Control3, IN_uv_Control);
		#endif
		...
		//
			...
			float4 splat_controlA = IN.color;
				...
				float4 splat_controlA = UNITY_SAMPLE_TEX2D_GRAD(_Control3, IN_uv_Control, _ddxGlobal, _ddyGlobal);
			#else
				float4 splat_controlA = UNITY_SAMPLE_TEX2D(_Control3, IN_uv_Control); 
			#endif
		#endif
		...
		#else
			#if (defined(RTP_STANDALONE) || defined(GEOM_BLEND)) && !SHADER_API_OPENGL
				float4 splat_controlB = UNITY_SAMPLE_TEX2D_GRAD_SAMPLER(_Control2, _Control3, IN_uv_Control, _ddxGlobal, _ddyGlobal);
			#else
				float4 splat_controlB = UNITY_SAMPLE_TEX2D_SAMPLER(_Control2, _Control3, IN_uv_Control);
			#endif
		 	total_coverage+=dot(splat_controlB, 1);
			//float4 splat_controlA_normalized=splat_controlA/total_coverage;
	...
	mip_selector=0.5*log2(d);
		...
		fixed4 hMapVal_YZ = tex2Dd(_TERRAIN_HeightMap3, _TERRAIN_HeightMap3,_INPUT_uv.yz, _ddxMain.yz, _ddyMain.yz);
		fixed4 hMapVal_XY = tex2Dd(_TERRAIN_HeightMap3, _TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy);
		fixed4 hMapVal_XZ = tex2Dd(_TERRAIN_HeightMap3, _TERRAIN_HeightMap3, _INPUT_uv.xz, _ddxMain.xz, _ddyMain.xz);
...
//	return;
	...
	#else
		#ifdef USE_EXTRUDE_REDUCTION
			tHA=saturate(lerp(tex2Dd(_TERRAIN_HeightMap3, _TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy), 1, PER_LAYER_HEIGHT_MODIFIER89AB)+0.001);
		#else
			tHA=saturate(tex2Dd(_TERRAIN_HeightMap3, _TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy)+0.001);
		#endif	
	#endif
	splat_control1 *= tHA;
		...
		float4 splat_control2 = splat_controlB;
		#ifdef USE_EXTRUDE_REDUCTION
			tHB=saturate(lerp(tex2Dd(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy), 1, PER_LAYER_HEIGHT_MODIFIER4567)+0.001);
			splat_control2 *= tHB;
		#else
			tHB=saturate(tex2Dd(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy)+0.001);
			splat_control2 *= tHB;
		#endif	
		float4 splat_control2_mid=splat_control2*splat_control2;
	...
	float splat_controlB_coverage=dot(splat_control2, 1);
		...
		#else
			float4 splat_control2;
			float4 splat_control2_mid;
			float4 splat_control2_close;
			{
			float4 splat_control1 = tHA*splat_controlA; // 2 (4-7)

			tHB=saturate(tex2Dd(_TERRAIN_HeightMap, _TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy)+0.001); // 1 (0-3)
			#if (defined(RTP_STANDALONE) || defined(GEOM_BLEND)) && !SHADER_API_OPENGL
				float4 splat_controlB = UNITY_SAMPLE_TEX2D_GRAD_SAMPLER(_Control1, _Control3, IN.uv_Control, _ddxGlobal, _ddyGlobal);
			#else
				float4 splat_controlB = UNITY_SAMPLE_TEX2D_SAMPLER(_Control1, _Control3, IN.uv_Control);
			#endif
			splat_control2 = tHB*splat_controlB;
			
			float4 splat_control1_mid=splat_control1*splat_control1;
			splat_control2_mid=splat_control2*splat_control2;
	 		...
	 		o.Emission.r=1;
				...
				for(int i=0; i<_TERRAIN_DIST_STEPS; i++) {
					rayPos.xyz+=EyeDirTan;
			 		float4 tH1, tH2;
					#ifdef USE_EXTRUDE_REDUCTION
						tH1=lerp(UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos.xyww), 1, PER_LAYER_HEIGHT_MODIFIER89AB);
						tH2=lerp(UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos.xyww), 1, PER_LAYER_HEIGHT_MODIFIER4567);
					#else
						tH1= UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos.xyww);
						tH2= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos.xyww);
					#endif
...
//#endif		 	
			...
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, _MipActual.xx)); col = splat_control1.x * c.rgb; gloss.r = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, _MipActual.xx)); col += splat_control1.y * c.rgb; gloss.g = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, _MipActual.xx)); col += splat_control1.z * c.rgb; gloss.b = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, _MipActual.xx)); col += splat_control1.w * c.rgb; gloss.a = c.a;
			...
			float glcombined = dot(gloss, splat_control1);
				...
				float4 _multMix89AB=_multMix-hi_mip_adjustMix; // nie chcemy nadpisac tego bo ponizej potrzeba nam tego dla kanaow 4-7
			...
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.xy, _MipActual.xx)); col += splat_control2.x * c.rgb; gloss.r = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.zw, _MipActual.xx)); col += splat_control2.y * c.rgb; gloss.g = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.xy, _MipActual.xx)); col += splat_control2.z * c.rgb; gloss.b = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.zw, _MipActual.xx)); col += splat_control2.w * c.rgb; gloss.a = c.a;

			glcombined += dot(gloss, splat_control2);
						
			#ifdef RTP_UV_BLEND
			#ifndef RTP_DISTANCE_ONLY_UV_BLEND
				_MixMipActual=min(mip_selector.xxxx + rtp_mipoffset_color + MIPmult4567 + log2(MixScaleRouted4567), float4(6,6,6,6)); // na layerach 4-7 w trybie 8 layers nie mozemy korzystac z UV blendu kanaow 4-7 (bo nie wiemy w define pobierajcym tekstur ponizej czy to pierwszy czy drugi set layerow) dlatego uzywamy tutaj maski 89AB
				_MixMipActual=max(float4(0,0,0,0),_MixMipActual); // nie moze byc ujemny do obj hi_mip_adjustMix ponizej
			...
			float glossMin = dot(splat_control1, RTP_glossMin89AB) + dot(splat_control2, RTP_glossMin4567);
			float glossMax = dot(splat_control1, RTP_glossMax89AB) + dot(splat_control2, RTP_glossMax4567);
			o.Metallic = dot(splat_control1, RTP_metallic89AB) + dot(splat_control2, RTP_metallic4567);
			...
			rayPos.w=mip_selector.x+rtp_mipoffset_bump;
				...
				rayPos.w += snow_depth;
			#endif				
			normals_combined = UNITY_SAMPLE_TEX2D_LOD(_BumpMap89, rayPos.xyww).rgba*splat_control1.rrgg;
			normals_combined+= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMapAB, _BumpMap89, rayPos.xyww).rgba*splat_control1.bbaa;
			normals_combined+= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap45, _BumpMap89, rayPos.xyww).rgba*splat_control2.rrgg;
			normals_combined+= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap67, _BumpMap89, rayPos.xyww).rgba*splat_control2.bbaa;
			...
			o_NormalNoWater = o.Normal;
					...
					half3 vert_tex= UNITY_SAMPLE_TEX2D_LOD(TERRAIN_CausticsTex, float4(vert_tex_uv, mip_selector-log2( TERRAIN_CausticsTex_TexelSize.y/(_SplatC0_TexelSize.x*(_TERRAIN_ReliefTransformTriplanarZ/_VerticalTextureTiling)) ))).rgb;
				#else
					half3 vert_tex= UNITY_SAMPLE_TEX2D_LOD(_VerticalTexture, float4(vert_tex_uv, mip_selector-log2( _VerticalTexture_TexelSize.y/(_SplatC0_TexelSize.x*(_TERRAIN_ReliefTransformTriplanarZ/_VerticalTextureTiling)) ))).rgb;
				#endif
			...
			//
				...
				{
					float Metallic_Water = dot(splat_control1, TERRAIN_WaterMetallic89AB) + dot(splat_control2, TERRAIN_WaterMetallic4567);
...
//#endif
			// snow color
				...
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_4)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.xy, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_1)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif			
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_5)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.zw, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif			
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_2)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif				
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_6)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.xy, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif			
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_3)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif				
			#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_7)
				half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.zw, min(mip_selector + rtp_mipoffset_color, 6)));
				GETrtp_snow_TEX
			#endif		
			#endif
			// snow color
			...
			IN_uv_Relief_Offset.xy=rayPos.xy;
		 	...
		 	//
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					...
					_h=dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww))+dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww));
					...
					if (hit_flag) {
						...
						{			
							...
							dh = saturate( dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww))+dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z ); // weight 1 (mip+1 frequency)
							
							rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade*3)*EyeDirTan.xyz; rayPos_shadows.w++;
							dh += saturate( dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww))+dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z )*4; // weight 4 (mip+2 frequency)
							
							rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade*7)*EyeDirTan.xyz; rayPos_shadows.w++;
							dh += saturate( dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww))+dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z )*8; // weight 8 (mip+3 frequency)
	 		...
	 		//////////////////////////////////
				...
				for(int i=0; i<_TERRAIN_DIST_STEPS; i++) {
					rayPos.xyz+=EyeDirTan;
			 		float4 tH;
					#ifdef USE_EXTRUDE_REDUCTION
						tH=lerp(UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos.xyww), 1, PER_LAYER_HEIGHT_MODIFIER89AB);
					#else
						tH= UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos.xyww);
					#endif	
				...
				if (hit_flag) {
					...
					actH=lerp(actH, rayPos.z, _uv_Relief_z);
		 				...
		 				if (triplanar_blend_flag) {
		 					...
		 					// no blend case
							#ifdef USE_EXTRUDE_REDUCTION
					 			hgtTRI = dot(splat_control1, lerp(UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, uvTRI.xyzz), 1, PER_LAYER_HEIGHT_MODIFIER89AB));
							#else
					 			hgtTRI = dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, uvTRI.xyzz));
							#endif
...
//#endif		 			
			...
			float4 gloss=0;
				...
				if (triplanar_blend_flag) {
					...
					if (triplanar_blend.x>0.02) {
						...
						dir.y=triplanar_flip.x ? -dir.y:dir.y;
							...
							uvSplat01.zw+=float2(0.5,0);
							uvSplat23=uvSplat01.xyxy+float4(0,0.5,0.5,0.5);
							c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, _MipActual.xx)); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r = c.a;
							c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, _MipActual.xx)); col += _MixBlendtmp.y  * c.rgb; tmp_gloss.g = c.a;
							#if !defined(USE_2_LAYERS_ONLY)
							c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, _MipActual.xx)); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b = c.a;
							#if !defined(USE_3_LAYERS_ONLY)
							c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, _MipActual.xx)); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a = c.a;
							#endif
							#endif							
						#else
							c = tex2Dd(_SplatC0, _SplatC0, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r=c.a;
							c = tex2Dd(_SplatC1, _SplatC0, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.y * c.rgb; tmp_gloss.g=c.a;
							#if !defined(USE_2_LAYERS_ONLY)
							c = tex2Dd(_SplatC2, _SplatC0, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b=c.a;
							#if !defined(USE_3_LAYERS_ONLY)
							c = tex2Dd(_SplatC3, _SplatC0, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a=c.a;
							#endif
							#endif
						#endif				
						...
						normals_combined = tex2Dd(_BumpMap89, _BumpMap89, uvTRI1.xy, _ddxMain.yz*snow_depthMult, _ddyMain.yz*snow_depthMult).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMapAB, _BumpMap89, uvTRI1.xy, _ddxMain.yz*snow_depthMult, _ddyMain.yz*snow_depthMult).rgba*splat_control1.bbaa;
						#endif							
					...
					if (triplanar_blend.y>0.02) {
						...
						dir.y=triplanar_flip.y ? -dir.y:dir.y;
							...
							#endif							
						#else
							c = tex2Dd(_SplatC0, _SplatC0, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r=c.a;
							c = tex2Dd(_SplatC1, _SplatC0, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.y * c.rgb; tmp_gloss.g=c.a;
							#if !defined(USE_2_LAYERS_ONLY)
							c = tex2Dd(_SplatC2, _SplatC0, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b=c.a;
							#if !defined(USE_3_LAYERS_ONLY)
							c = tex2Dd(_SplatC3, _SplatC0, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a=c.a;
							#endif
							#endif	
						#endif				
						...
						normals_combined = tex2Dd(_BumpMap89, _BumpMap89, uvTRI2.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMapAB, _BumpMap89, uvTRI2.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.bbaa;
						#endif							
					...
					if (triplanar_blend.z>0.02) {
						...
						dir.y=triplanar_flip.z ? dir.y:-dir.y;
							...
							#endif							
						#else
							c = tex2Dd(_SplatC0, _SplatC0, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r=c.a;
							c = tex2Dd(_SplatC1, _SplatC0, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.y * c.rgb; tmp_gloss.g=c.a;
							#if !defined(USE_2_LAYERS_ONLY)
							c = tex2Dd(_SplatC2, _SplatC0, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b=c.a;
							#if !defined(USE_3_LAYERS_ONLY)
							c = tex2Dd(_SplatC3, _SplatC0, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a=c.a;
							#endif
							#endif							
						#endif
						...
						normals_combined = tex2Dd(_BumpMap89, _BumpMap89, uvTRI3.xy, _ddxMain.xz*snow_depthMult, _ddyMain.xz*snow_depthMult).rgba*splat_control1.rrgg;
						#if !defined(USE_2_LAYERS_ONLY)
						normals_combined+=tex2Dd(_BumpMapAB, _BumpMap89, uvTRI3.xy, _ddxMain.xz*snow_depthMult, _ddyMain.xz*snow_depthMult).rgba*splat_control1.bbaa;
						#endif							
					...
					float4 tmp_gloss=0;
						...
						uvSplat01.zw+=float2(0.5,0);
						uvSplat23=uvSplat01.xyxy+float4(0,0.5,0.5,0.5);
						c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, _MipActual.xx)); col += c.rgb*splat_control1.x; tmp_gloss.r = c.a;
						c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, _MipActual.xx)); col += c.rgb*splat_control1.y; tmp_gloss.g = c.a;
						#if !defined(USE_2_LAYERS_ONLY)
						c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, _MipActual.xx)); col += c.rgb*splat_control1.z; tmp_gloss.b = c.a;
						#if !defined(USE_3_LAYERS_ONLY)
						c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, _MipActual.xx)); col += c.rgb*splat_control1.w; tmp_gloss.a = c.a;
						#endif
						#endif							
					#else
						c = tex2Dd(_SplatC0, _SplatC0, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
						c = tex2Dd(_SplatC1, _SplatC0, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
						#if !defined(USE_2_LAYERS_ONLY)
						c = tex2Dd(_SplatC2, _SplatC0, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
						#if !defined(USE_3_LAYERS_ONLY)
						c = tex2Dd(_SplatC3, _SplatC0, uvTRI_tmp.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
						#endif
						#endif			
					#endif						
					gloss=tmp_gloss;
					...
					float4 normA=tex2Dd(_BumpMap89, _BumpMap89, uvTRI_tmp.xy, _ddxMainTRI.xy*snow_depthMult, _ddyMainTRI.xy*snow_depthMult).rgba;
					float4 normals_combined = normA*splat_control1.rrgg;
					#if !defined(USE_2_LAYERS_ONLY)
					float4 normB=tex2Dd(_BumpMapAB, _BumpMap89, uvTRI_tmp.xy, _ddxMainTRI.xy*snow_depthMult, _ddyMainTRI.xy*snow_depthMult).rgba;
				...
				//
					...
					c = tex2Dd(_SplatC0, _SplatC0, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col = splat_control1.x * c.rgb; gloss.r = c.a;
					c = tex2Dd(_SplatC1, _SplatC0, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col += splat_control1.y * c.rgb; gloss.g = c.a;
					#if !defined(USE_2_LAYERS_ONLY)
					c = tex2Dd(_SplatC2, _SplatC0, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col += splat_control1.z * c.rgb; gloss.b = c.a;
					#if !defined(USE_3_LAYERS_ONLY)
					c = tex2Dd(_SplatC3, _SplatC0, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col += splat_control1.w * c.rgb; gloss.a = c.a;
					#endif
					#endif							
				#endif
			...
			glcombined=saturate(glcombined - glcombined*dot(splat_control1, RTP_AO_89AB) * saturate((mip_selector.x+rtp_mipoffset_color)*0.2) );
			float glossMin = dot(splat_control1, RTP_glossMin89AB);
			float glossMax = dot(splat_control1, RTP_glossMax89AB);
			o.Metallic = dot(splat_control1, RTP_metallic89AB);
			o.Smoothness = lerp(glossMin, glossMax, glcombined);
			...
			col*=dot(splat_control1, _LayerBrightness89AB);  
				...
				rayPos.w=mip_selector.x+rtp_mipoffset_bump;
					...
					rayPos.w += snow_depth;
				#endif				
				normals_combined = tex2Dd(_BumpMap89, _BumpMap89, rayPos.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.rrgg;
				#if !defined(USE_2_LAYERS_ONLY)
				normals_combined+=tex2Dd(_BumpMapAB, _BumpMap89, rayPos.xy, _ddxMain.xy*snow_depthMult, _ddyMain.xy*snow_depthMult).rgba*splat_control1.bbaa;
				#endif							
				n.xy=(normals_combined.rg+normals_combined.ba)*2-1;
				n.xy*=_uv_Relief_z;
				n.z = sqrt(1 - saturate(dot(n.xy, n.xy)));
			...
			//
				...
				o.Smoothness = max(o.Smoothness, lerp(o.Smoothness, lerp(TERRAIN_WetGloss, TERRAIN_WaterGloss, p)*glossDamper, wet_fct)); // water glossiness
				{
					float Metallic_Water = dot(splat_control1, TERRAIN_WaterMetallic89AB);
					o.Metallic = max(o.Metallic, lerp(o.Metallic, Metallic_Water, wet_fct));
				}
		  		// col - saturation, brightness
		  		half3 col_sat=col.rgb*col.rgb + half3(0.001, 0.001, 0.001); // saturation z utrzymaniem jasnosci
...
//#endif
			// snow color
				...
				rayPos.w=mip_selector.x+rtp_mipoffset_color;
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_4)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB0, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_0)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD(_SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_5)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB1, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_1)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC1, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_6)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB2, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_2)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC2, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_7)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB3, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_3)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC3, _SplatC0, rayPos.xyww+SNOW_PM_OFFSET);
					GETrtp_snow_TEX
				#endif
			#endif	
			#endif
			...
			IN_uv_Relief_Offset.xy=rayPos.xy;
		 	...
		 	//
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					rayPos_shadows.xyz+=EyeDirTan;
					_h=dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww));
					hit_flag=_h >= rayPos_shadows.z;
					if (hit_flag) break;
					h_prev=_h;
					dh_prev = rayPos_shadows.z - _h;
					...
					if (hit_flag) {
						...
						{			
							...
							rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade)*EyeDirTan.xyz; rayPos_shadows.w++;
							dh = saturate( dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z ); // weight 1 (mip+1 frequency)
							
							rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade*3)*EyeDirTan.xyz; rayPos_shadows.w++;
							dh += saturate( dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z )*4; // weight 4 (mip+2 frequency)
							
							rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade*7)*EyeDirTan.xyz; rayPos_shadows.w++;
							dh += saturate( dot(splat_control1, UNITY_SAMPLE_TEX2D_LOD(_TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z )*8; // weight 8 (mip+3 frequency)
	 		...
	 		//////////////////////////////////
				...
				for(int i=0; i<_TERRAIN_DIST_STEPS; i++) {
					rayPos.xyz+=EyeDirTan;
			 		float4 tH;
					#ifdef USE_EXTRUDE_REDUCTION
						tH=lerp(UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos.xyww), 1, PER_LAYER_HEIGHT_MODIFIER4567);
					#else
						tH= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos.xyww);
					#endif						
...
//#endif
			...
			// eof snow color
		 	...
		 	//
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					rayPos_shadows.xyz+=EyeDirTan;
					_h=dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww));
					hit_flag=_h >= rayPos_shadows.z;
					if (hit_flag) break;
					h_prev=_h;
					dh_prev = rayPos_shadows.z - _h;
						...
						if (hit_flag) {
							...
							{			
								...
								rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade)*EyeDirTan.xyz; rayPos_shadows.w++;
								dh = saturate( dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z ); // weight 1 (mip+1 frequency)
								
								rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade*3)*EyeDirTan.xyz; rayPos_shadows.w++;
								dh += saturate( dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z )*4; // weight 4 (mip+2 frequency)
								
								rayPos_shadows.xyz += (1+_TERRAIN_ShadowSoftnessFade*7)*EyeDirTan.xyz; rayPos_shadows.w++;
								dh += saturate( dot(splat_control2, UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_TERRAIN_HeightMap2, _TERRAIN_HeightMap3, rayPos_shadows.xyww)) - rayPos_shadows.z )*8; // weight 8 (mip+3 frequency)
...
//#endif			
			...
			float4 gloss;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, _MipActual.xx)); col = splat_control1.x * c.rgb; gloss.r = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, _MipActual.yy)); col += splat_control1.y * c.rgb; gloss.g = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, _MipActual.zz)); col += splat_control1.z * c.rgb; gloss.b = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, _MipActual.ww)); col += splat_control1.w * c.rgb; gloss.a = c.a;
			...
			float glcombined = dot(gloss, splat_control1);
				...
				_MixMipActual=max(float4(0,0,0,0),_MixMipActual); // nie moze byc ujemny do obj hi_mip_adjustMix ponizej
				float4 hi_mip_adjustMix=exp2(_MixMipActual)*_SplatAtlasC_TexelSize.x;
				float4 _multMix89AB=_multMix - hi_mip_adjustMix; // _multMix / _offMix potrzebujemy ponizej dla kanaow 4-7
				float4 _offMix89AB=_offMix + 0.5*hi_mip_adjustMix;
			...
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.xy, _MipActual.xx)); col += splat_control2.x * c.rgb; gloss.r = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.zw, _MipActual.yy)); col += splat_control2.y * c.rgb; gloss.g = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.xy, _MipActual.zz)); col += splat_control2.z * c.rgb; gloss.b = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.zw, _MipActual.ww)); col += splat_control2.w * c.rgb; gloss.a = c.a;
			glcombined += dot(gloss, splat_control2);
...
//#endif		
			...
			float4 gloss=0;			
				...
				float4 tmp_gloss=0;
					...
					uvSplat01.zw+=float2(0.5,0);
					uvSplat23=uvSplat01.xyxy+float4(0,0.5,0.5,0.5);
					c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, _MixMipActual.xx)); col += c.rgb*splat_control1.x; tmp_gloss.r = c.a;
					c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, _MixMipActual.yy)); col += c.rgb*splat_control1.y; tmp_gloss.g = c.a;
					#if !defined(USE_2_LAYERS_ONLY)
					c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, _MixMipActual.zz)); col += c.rgb*splat_control1.z; tmp_gloss.b = c.a;
					#if !defined(USE_3_LAYERS_ONLY)
					c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, _MixMipActual.ww)); col += c.rgb*splat_control1.w; tmp_gloss.a = c.a;
					#endif
					#endif							
				#else
					float4 mipTweak=exp2(MIPmult89AB);				
					c = tex2Dd(_SplatC0, _SplatC0, uvTRI.xy, _ddxMainTRI.xy*mipTweak.x, _ddyMainTRI.xy*mipTweak.x); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
					c = tex2Dd(_SplatC1, _SplatC0, uvTRI.xy, _ddxMainTRI.xy*mipTweak.y, _ddyMainTRI.xy*mipTweak.y); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
					#if !defined(USE_2_LAYERS_ONLY)
					c = tex2Dd(_SplatC2, _SplatC0, uvTRI.xy, _ddxMainTRI.xy*mipTweak.z, _ddyMainTRI.xy*mipTweak.z); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
					#if !defined(USE_3_LAYERS_ONLY)
					c = tex2Dd(_SplatC3, _SplatC0, uvTRI.xy, _ddxMainTRI.xy*mipTweak.w, _ddyMainTRI.xy*mipTweak.w); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
					#endif
					#endif		
				#endif						
				gloss=tmp_gloss;					
				...
				//
					...
					c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.xy, _MixMipActual.xx)); col = splat_control1.x * c.rgb; gloss.r = c.a;
					c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat01.zw, _MixMipActual.yy)); col += splat_control1.y * c.rgb; gloss.g = c.a;
					#if !defined(USE_2_LAYERS_ONLY)
						c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.xy, _MixMipActual.zz)); col += splat_control1.z * c.rgb; gloss.b = c.a;
						#if !defined(USE_3_LAYERS_ONLY)
							c = UNITY_SAMPLE_TEX2D_LOD(_SplatAtlasC, float4(uvSplat23.zw, _MixMipActual.ww)); col += splat_control1.w * c.rgb; gloss.a = c.a;
						#endif
					#endif							
				#else
					float4 _MixMipActual=mip_selector.xxxx + rtp_mipoffset_color+MIPmult89AB;
					float4 mipTweak=exp2(MIPmult89AB);
					c = tex2Dd(_SplatC0, _SplatC0, _INPUT_uv.xy, _ddxMain.xy*mipTweak.x, _ddyMain.xy*mipTweak.x); col = splat_control1.x * c.rgb; gloss.r = c.a;
					c = tex2Dd(_SplatC1, _SplatC0, _INPUT_uv.xy, _ddxMain.xy*mipTweak.y, _ddyMain.xy*mipTweak.y); col += splat_control1.y * c.rgb; gloss.g = c.a;
					#if !defined(USE_2_LAYERS_ONLY)
						c = tex2Dd(_SplatC2, _SplatC0, _INPUT_uv.xy, _ddxMain.xy*mipTweak.z, _ddyMain.xy*mipTweak.z); col += splat_control1.z * c.rgb; gloss.b = c.a;
						#if !defined(USE_3_LAYERS_ONLY)
							c = tex2Dd(_SplatC3, _SplatC0, _INPUT_uv.xy, _ddxMain.xy*mipTweak.w, _ddyMain.xy*mipTweak.w); col += splat_control1.w * c.rgb; gloss.a = c.a;
						#endif
					#endif							
				#endif
...
//#endif			
			...
			#else
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_4)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB0, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_0)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD(_SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_5)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB1, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_1)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC1, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_6)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB2, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_2)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC2, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_7)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatB3, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
				#if defined(RTP_SNW_CHOOSEN_LAYER_COLOR_3)
					half4 csnow = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatC3, _SplatC0, float4(_INPUT_uv.xy, mip_selector + rtp_mipoffset_color));
					GETrtp_snow_TEX
				#endif
			#endif	
			#endif
...
//#endif		
			...
			half4 c;
			float4 gloss;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.xy, _MixMipActual.xx)); col = splat_control2.x * c.rgb; gloss.r = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat01.zw, _MixMipActual.yy)); col += splat_control2.y * c.rgb; gloss.g = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.xy, _MixMipActual.zz)); col += splat_control2.z * c.rgb; gloss.b = c.a;
			c = UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_SplatAtlasB, _SplatAtlasC, float4(uvSplat23.zw, _MixMipActual.ww)); col += splat_control2.w * c.rgb; gloss.a = c.a;
...
//#endif		
	...
	#if defined(RTP_UV_BLEND) && defined(RTP_NORMALS_FOR_REPLACE_UV_BLEND)
	{
		...
		float4 mipN=mip_selector.xxxx+rtp_mipoffset_bump+MIPmult89AB+log2(MixScaleRouted89AB);
			...
			mipN += snow_depth;
		#endif
		n.xy = UNITY_SAMPLE_TEX2D_LOD(_BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted89AB.x,  mipN.xx)).rg*splat_control1.r;
		n.xy += UNITY_SAMPLE_TEX2D_LOD(_BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted89AB.y,  mipN.yy)).ba*splat_control1.g;
		n.xy += UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMapAB, _BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted89AB.z,  mipN.zz)).rg*splat_control1.b;
		n.xy += UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMapAB, _BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted89AB.w,  mipN.ww)).ba*splat_control1.a;
		#ifndef _4LAYERS
			mipN=mip_selector.xxxx+rtp_mipoffset_bump+MIPmult4567+log2(MixScaleRouted4567);
			#if ( defined(RTP_SNOW) && !defined(UNITY_PASS_SHADOWCASTER) )
				mipN += snow_depth;
			#endif
			n.xy += UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap45, _BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted4567.x,  mipN.xx)).rg*splat_control2.r;
			n.xy += UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap45, _BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted4567.y,  mipN.yy)).ba*splat_control2.g;
			n.xy += UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap67, _BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted4567.z,  mipN.zz)).rg*splat_control2.b;
			n.xy += UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap67, _BumpMap89, float4(_INPUT_uv.xy*MixScaleRouted4567.w,  mipN.ww)).ba*splat_control2.a;
		#endif
		n.xy=n.xy*2-1;
		n.z = sqrt(1 - saturate(dot(n.xy, n.xy)));
		#ifdef RTP_TRIPLANAR
	...
	float _BumpMapGlobalStrengthPerLayer=1;
	...
	#ifdef _4LAYERS
		o.Smoothness = lerp( saturate(o.Smoothness + dot(splat_control1_nobleed, _FarSpecCorrection89AB)), o.Smoothness, (1-_uv_Relief_w)*(1-_uv_Relief_w) );
	#else
		o.Smoothness = lerp( saturate(o.Smoothness + (dot(splat_control1_nobleed, _FarSpecCorrection89AB) + dot(splat_control2_nobleed, _FarSpecCorrection4567))), o.Smoothness, (1-_uv_Relief_w)*(1-_uv_Relief_w) );
	#endif
		...
		float colBrightness=dot(col,1);
			...
			o.Normal=lerp(o.Normal, float3(0,0,1), edge_lerp);
					...
					half3 vert_tex= UNITY_SAMPLE_TEX2D_LOD(_VerticalTexture, float4(vert_tex_uv, mip_selector-log2( _VerticalTexture_TexelSize.y/(_SplatC0_TexelSize.x*(_TERRAIN_ReliefTransformTriplanarZ/_VerticalTextureTiling)) ))).rgb;
				#endif
				fixed3 colV=lerp(global_color_value.rgb, global_color_value.rgb*vert_tex*2, dot(splat_control1, _VerticalTexture89AB));
		...
		col=lerp( col, rtp_snow_color_tex, snow_val );
			...
			float3 n;
				...
				n.xy= UNITY_SAMPLE_TEX2D_LOD(_BumpMap89, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_1
				n.xy= UNITY_SAMPLE_TEX2D_LOD(_BumpMap89, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_2
				n.xy= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMapAB, _BumpMap89, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_3
				n.xy= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMapAB, _BumpMap89, float4(IN_uv_Relief_Offset.xy+SNOW_PM_OFFSET.xy, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_4
				n.xy= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap45, _BumpMap89, float4(IN_uv_Relief_Offset.xy, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_5
				n.xy= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap45, _BumpMap89, float4(IN_uv_Relief_Offset.xy, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_6
				n.xy= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap67, _BumpMap89, float4(IN_uv_Relief_Offset.xy, mip_selector + rtp_mipoffset_bump)).rg*2-1;
			#endif
			#ifdef RTP_SNW_CHOOSEN_LAYER_NORM_7
				n.xy= UNITY_SAMPLE_TEX2D_LOD_SAMPLER(_BumpMap67, _BumpMap89, float4(IN_uv_Relief_Offset.xy, mip_selector + rtp_mipoffset_bump)).ba*2-1;
			#endif
			n.xy*=_uv_Relief_z;
			n.z = sqrt(1 - saturate(dot(n.xy, n.xy)));
		...
		// zamienione rtp_snow_specular z rtp_snow_gloss, podmien ponizsz sekcj (wylecia m. in. GlossDry)
	...
	// emission of layer (glow)
		...
		#ifdef _4LAYERS
			float glitter_thickness = dot(splat_control1, _GlitterStrength89AB);
		#else
			float glitter_thickness = dot(splat_control1, _GlitterStrength89AB) + dot(splat_control2, _GlitterStrength4567);
		#endif
		#if defined(RTP_TRIPLANAR)
...
//	return;
	...
	// crosspass heightblend ao only for dx10+ hardware - d3d9 hlsl compiler runs out of temp registers... :(
	#if defined(RTP_CROSSPASS_HEIGHTBLEND) && (defined(SHADER_API_D3D11) || defined(SHADER_API_XBOXONE) || defined(SHADER_API_GLES3) || defined(SHADER_API_GLCORE) || defined(SHADER_API_PSSL))
		#if !defined(_12LAYERS)
			#ifdef RTP_HEIGHTBLEND_AO
			// 0-3			
			...
			//o.atten*=heightblend_AO;
			o.Albedo *= heightblend_AO; // to influence direct and ambient light
